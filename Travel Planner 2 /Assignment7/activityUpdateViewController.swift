...
class activityUpdateViewController: UIViewController {
    ...
    func setupUI() {
        ...
        if let date = dateFormatter.date(from: activity.date) {
            datePicker.date = date
        }
        
        let timeFormatter = DateFormatter()
        timeFormatter.dateFormat = "hh:mm a"
        if let time = timeFormatter.date(from: activity.time) {
    ...
    @IBAction func updateActivityTapped(_ sender: UIButton) {
        ...
        if let activity = activity {
            DataManager.shared.updateActivity(id: activity.id, newName: updatedName, newDate: updatedDate, newTime: updatedTime, newLocation: updatedLocation)
        }
